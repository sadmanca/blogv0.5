...
There's two reasons why I'm interested in terminal session recorders:
...
\*More on point 2, converting recordings from `asciicast` to GIF/SVG is where `asciinema` falls flat (it only supports its proprietary), which is where middleware like `agg` and `PowerSession-rs` come in (and I'll be taking about both at length later on).

[^videolens]: Of which virtually all are expected to be up to only a couple seconds in length
